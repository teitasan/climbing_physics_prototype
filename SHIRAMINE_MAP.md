...
地面の傾斜が約35°を超えると、歩行・スプリントをMixamoの `Crawling — Crawling Forward On Hands And Knees`（`assets/motions/mixamo/Crawling.fbx`）へ自動で切り替える。移動速度は約1.46m/sに落とし、キャラクターの体も地面の法線へ滑らかに沿わせる。約30°未満へ戻ると直立歩行へ戻る。切り替えにはヒステリシスを入れて、地形の三角形境界で姿勢がちらつかないようにしている。`debug=1` ではHUDに現在の傾斜角と状態（`crawl`）が出る。
