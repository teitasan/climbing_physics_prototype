...
歩行速度の調整を試すときは `shiramine.html?debug=1` を開く。右上の「歩行速度」スライダーを 0.8〜3.2m/s の範囲で動かせ、既定値は 1.8m/s。URL に `&walkSpeed=2.1` のように指定して初期値を渡すこともできる。速度に合わせて歩行アニメの再生倍率も変わるため、足滑りを比較しやすい。
